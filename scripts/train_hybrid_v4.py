"""CLI script for training Hybrid V4 RGB+FFT with positive class weighting (pos_weight=1.35).

Strict single-variable experiment relative to Hybrid V1:
...
4. Exact V1 augmentations (NO horizontal flip, NO multi-scale crop).
5. ONLY change: BCEWithLogitsLoss(pos_weight=torch.tensor([1.35])).
6. Target checkpoint: models/convnext_tiny_hybrid_v4_weighted.pt.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Hybrid V4 RGB+FFT with pos_weight=1.35 on development split."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        "--checkpoint-out",
        type=Path,
        default=Path("models/convnext_tiny_hybrid_v4_weighted.pt"),
        help="Output path for best checkpoint.",
    )
    parser.add_argument(
        "--report-out",
        type=Path,
        default=Path("reports/hybrid_v4_clean_eval.json"),
...
def main() -> int:
    ...
    print("=================================================================")
    print(" Verixa — Training Hybrid V4 Final Candidate Model")
    print("=================================================================")
    ...
    print(" Horizontal Flip:   None (Identical to V1)")
    print(f" Loss Function:     BCEWithLogitsLoss(pos_weight={args.pos_weight})")
    ...
    # 1. Dataloaders with identical V1 transformation-aware augmentations (NO flip)
    ...
    results = fit_convnext_baseline(
        ...
        config=run_config,
        patience=args.patience,
        model_name="HybridRGBFFT-V4-Weighted",
        criterion=criterion,
    )
    ...
    payload_str = json.dumps(report_payload, indent=2, sort_keys=True)
    args.report_out.write_text(payload_str, encoding="utf-8")
    print(f"Saved clean training report ({len(payload_str):,} bytes) to: {args.report_out}")
    return 0
