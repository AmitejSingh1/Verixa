...
- **Current Phase:** **Phase 2: RGB ConvNeXt-Tiny Baseline Model Training — COMPLETED**
- **Completed Work:**
...
- **In Progress / Current Stage:**
  - Ready for Git commit of Phase 2 deliverables and transition to Phase 3.
- **Not Started:**
  - **Phase 3 (Transformation-Aware Training):** Ready to begin.
  - **Phase 4 (FFT Dual-Branch Experiment):** Not started.
...
## 5. Official Phase 2 Baseline (Configuration & Verified Results)

The official Phase 2 baseline training on the **frozen 30K dataset** is **COMPLETED AND FULLY VERIFIED**.

### 5.1 Training Configuration
- **Model Backbone:** `convnext_tiny(weights=ConvNeXt_Tiny_Weights.IMAGENET1K_V1)` (~28.6M parameters)
- **Classification Head:** AdaptiveAvgPool2d $\rightarrow$ LayerNorm(768) $\rightarrow$ Linear(768, 256) $\rightarrow$ GELU $\rightarrow$ Dropout(0.3) $\rightarrow$ Linear(256, 1)
- **Stage Freezing:** Stages 0, 1, 2 frozen (~13.0M params); Stage 3 + head active (**15,669,249 trainable params**)
- **Data Normalization:** Standard ImageNet mean `[0.485, 0.456, 0.406]` and std `[0.229, 0.224, 0.225]`
- **Augmentations:** **None** (clean images only, no distortions, no FFT)
...
- **Batch Size:** 32
- **Precision:** Mixed Precision (`torch.amp` FP16 with `GradScaler`)
- **Maximum Epochs:** 20 (Early stopping patience = 4 on validation AUROC)
- **Model Selection Metric:** Highest Validation AUROC on the 6,001 validation split
- **Random Seed:** Globally locked to `1337`
- **Output Checkpoint:** [`models/convnext_tiny_baseline.pt`](file:///c:/Verixa/models/convnext_tiny_baseline.pt) (237.5 MB)
- **Output Report:** [`reports/baseline_clean_eval.json`](file:///c:/Verixa/reports/baseline_clean_eval.json)

### 5.2 Verified Official Baseline Results (Best Epoch 2)
- **Best Epoch:** **Epoch 2** (Early stopping triggered at Epoch 6 after 4 non-improving epochs: epochs 3–6)
- **Validation AUROC:** **0.9964 (99.64%)**
- **Clean Validation Accuracy:** **97.03%** (at default threshold 0.5)
- **Validation Loss:** **0.0920** (Train Loss at Epoch 2: 0.0709)
- **False Positive Rate (FPR):** **2.80%** (84 authentic images misclassified as AI out of 3,000)
- **False Negative Rate (FNR):** **3.13%** (94 synthetic images misclassified as authentic out of 3,001)
- **FPR at 95% TPR:** **1.63%**
- **Confusion Matrix:**
  $$\begin{pmatrix} \text{TN}: 2,916 & \text{FP}: 84 \\ \text{FN}: 94 & \text{TP}: 2,907 \end{pmatrix}$$
- **Hardware Profile:**
  - Peak VRAM Allocated: **545.08 MB** ($< 7\%$ of 8.0 GB budget)
  - Peak VRAM Reserved: **734.00 MB**
  - Average Epoch Duration: **~66.0 seconds** on RTX 4060 Laptop GPU
  - Total Training Runtime: **~6.8 minutes** (6 epochs total)
...
| [`src/verixa/training/dataset.py`](file:///c:/Verixa/src/verixa/training/dataset.py) | PyTorch Dataset and DataLoader reading from manifest CSV with ImageNet normalization |
...
| [`scripts/train_rgb.py`](file:///c:/Verixa/scripts/train_rgb.py) | CLI training script for ConvNeXt-Tiny baseline with full run-config embedding |
...
- **Automated Tests:** **20/20 passed** (`pytest -v`).
...
- **GPU Hardware:** NVIDIA GeForce RTX 4060 Laptop GPU (8,192 MB VRAM, measured baseline peak ~546 MB).
...
2. **Fast Fourier Transform (FFT) is Experimental:** Belongs strictly to Phase 4. It will only be added if it demonstrates a statistically significant robustness delta over the spatial model.
...
7. **Strict Head-to-Head Coupling:** Phase 3 (Transformation-Aware Training) must use the **exact same frozen 30K dataset and split** as the Phase 2 baseline so that the measured robustness delta is scientifically valid.
...
9. **Never Fabricate Metadata:** Generator architecture tags must only be recorded if present in verified dataset features.
...
- **Latest Completed Commit:** `30c8775` — `feat(model): complete Phase 2 RGB ConvNeXt-Tiny baseline training and evaluation`
- **Current Working Tree:** All Phase 2 criteria are satisfied. Files modified/ready to stage and commit:
  - `Architecture.md` (Updated with complete system architecture and 30K dataset structure)
  - `PRD.md` (Updated with 30K dataset specifications and WildFake exclusion)
  - `Phases.md` (Phase 2 tasks marked complete; Phase 3 coupled to identical 30K split)
  - `Memory.md` (Handoff file updated with verified 30K baseline metrics)
- **Commit Pending:**
  `feat(model): complete Phase 2 RGB ConvNeXt-Tiny baseline training on 30K dataset`

---

## 11. NEXT ACTION

The official Phase 2 baseline training on the 30K dataset is **complete, evaluated, and verified**.

The exact next steps are:
1. **Commit Phase 2 Deliverables:**
   Stage `Architecture.md`, `PRD.md`, `Phases.md`, `Memory.md`, and commit:
   ```bash
   git add Architecture.md PRD.md Phases.md Memory.md
   git commit -m "feat(model): complete Phase 2 RGB ConvNeXt-Tiny baseline training on 30K dataset"
   ```
2. **Proceed to Phase 3: Transformation-Aware Training + Decision Checkpoint #1:**
   - Implement `src/verixa/training/augmentations.py` covering the 6 required distortion families (JPEG compression $Q \in [30, 90]$, Gaussian blur $\sigma \in [0.5, 2.0]$, bilinear resize $0.25\times\text{--}0.5\times$, Gaussian noise $\sigma \in [0.02, 0.10]$, color jitter $\pm 20\%$, center crop $80\%$).
   - Integrate on-the-fly augmentation pipeline into DataLoader with probability $p=0.8$.
   - Train the robust ConvNeXt-Tiny model on the **identical frozen 30K dataset and split** (`data/manifests/merged_manifest.csv`) with seed `1337`.
   - Save checkpoint to `models/convnext_tiny_robust_rgb.pt`.
   - Benchmark both models across all 6 distortion suites and severities to compute $\Delta_{\text{degradation}}$ and reach **Decision Checkpoint #1**.
